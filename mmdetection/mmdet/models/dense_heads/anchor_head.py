...
class AnchorHead(BaseDenseHead, BBoxTestMixin):
    ...
    def _get_targets_single(self,
        ...
        if len(pos_inds) > 0:
            ...
            if 'confidences' in img_meta.keys():
                bbox_weights[pos_inds] = ((torch.tensor(img_meta['confidences']).to(bbox_weights.device))[sampling_result.pos_assigned_gt_inds]**(1/2)).unsqueeze(-1)
            else:
                if 'confidences' in img_meta.keys():
                    raise
            ...
            if self.train_cfg.pos_weight <= 0:
                if 'confidences' in img_meta.keys():
                    label_weights[pos_inds] = (torch.tensor(img_meta['confidences']).to(bbox_weights.device))[sampling_result.pos_assigned_gt_inds]**(1/2)
                else:
        ...
        if len(neg_inds) > 0:
            if 'confidences' in img_meta.keys():
                if 'test' in img_meta['ori_filename']:
                    label_weights[neg_inds] = 0.9
                else:
